...
st.markdown("""
    ...
    .alerta-colas { background-color: #DA291C; color: white; padding: 15px; border-radius: 8px; text-align: center; font-size: 20px; font-weight: bold; margin-bottom: 20px; animation: blinker 1s linear infinite; }
...
def render_full_view(data, mode="vivo"):
    # ALERTA DE COLA (SÓLO VIVO)
    if mode == "vivo":
        now = datetime.now(BOGOTA_TZ)
        if now.minute % 5 == 0:
            st.markdown(f'<div class="alerta-colas">🚨 HORA DE COLA ({now.strftime("%H:%M")}): ¡REGISTRA YA! 🚨</div>', unsafe_allow_html=True)
    ...
    with t2:
        with st.container(border=True):
            st.subheader("🍳 Medición de Estaciones")
            ...
            if data['stations']:
                st.write("**Historial de Estaciones:**")
    ...
    with t4:
        ...
        if data['orders']:
            ...
            if not df_ord.empty:
                ...
                st.plotly_chart(px.bar(fig_df.melt(id_vars='Franja', value_vars=resumen.columns), x='Franja', y='value', color='variable', color_discrete_map=MC_COLORS, barmode='stack', text_auto=True), use_container_width=True)
