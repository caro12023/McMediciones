...
def render_full_view(data, mode="vivo"):
    ...
    if mode == "vivo":
        ...
        # Definir el intervalo actual (ej: 12:05)
        interval_start = now.replace(second=0, microsecond=0) - timedelta(minutes=now.minute % 5)
        interval_str = interval_start.strftime("%H:%M")
        
        # Revisar si ya existe un registro de cola para esta franja
        already_saved = any(q.get('Intervalo') == interval_str for q in st.session_state.queues)
        
        if now.minute % 5 == 0 and not already_saved:
            st.markdown(f'<div class="alerta-colas">🚨 HORA DE COLA ({now.strftime("%H:%M")}): ¡REGISTRA Y GUARDA YA! 🚨</div>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        with st.container(border=True):
            ...
            if c_qb.button("💾 Guardar Cola", use_container_width=True):
                interval_now = datetime.now(BOGOTA_TZ).replace(second=0, microsecond=0) - timedelta(minutes=datetime.now(BOGOTA_TZ).minute % 5)
                st.session_state.queues.append({
                    "Hora Registro": datetime.now(BOGOTA_TZ).strftime("%H:%M:%S"),
                    "Intervalo": interval_now.strftime("%H:%M"),
                    "Caja": qc, 
                    "AutoMac": qa
                })
                st.rerun()
            if data['queues']:
                st.write("**Live Log: Colas Registradas**")
    ...
    with t2:
        with st.container(border=True):
            ...
            if data['stations']:
                ...
                ce = pd.DataFrame(data['stations'])[lambda x: x['Fase'] == 'Completado']
                if not ce.empty: st.dataframe(ce.iloc[::-1], use_container_width=True)
    ...
    with t4:
        ...
        if data['orders']:
            ...
            if not df_ord.empty:
                st.write("### 1. Demanda por Franja (5 min)")
                df_arr = df_ord[['Canal', 'Inicio']].copy()
                df_arr['Timestamp'] = pd.to_datetime(df_arr['Inicio'])
                dem_5m = df_arr.groupby([pd.Grouper(key='Timestamp', freq='5min'), 'Canal']).size().reset_index(name='Pedidos')
                resumen = dem_5m.pivot(index='Timestamp', columns='Canal', values='Pedidos').fillna(0).astype(int)
                ...
                cols_presentes = [c for c in ['Caja', 'AutoMac', 'Delivery/Pickup'] if c in resumen.columns]
                
                if not fig_df.empty and cols_presentes:
                    st.plotly_chart(px.bar(fig_df.melt(id_vars='Franja', value_vars=cols_presentes), x='Franja', y='value', color='variable', color_discrete_map=MC_COLORS, barmode='stack', text_auto=True), use_container_width=True)
                
                resumen['Total Intervalo'] = resumen.sum(axis=1)
                total_franja = pd.DataFrame(resumen.sum()).T; total_franja.index = ["TOTAL FRANJA"]
                resumen.index = [f"{t.strftime('%H:%M')} - {(t + timedelta(minutes=5)).strftime('%H:%M')}" for t in resumen.index]
                st.write("**Tabla de Demanda:**"); st.dataframe(pd.concat([resumen, total_franja]), use_container_width=True)
        else: st.info("Registra pedidos para ver el Dashboard.")
        
        if data['stations']:
            st.divider()
            st.write("### 2. Estadísticas")
            cs = pd.DataFrame(data['stations'])[lambda x: x['Fase'] == 'Completado']
            if not cs.empty:
...
if st.session_state.selected_history:
    ...
    st.title("🍔 McMediciones Sabana Pro")
