    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if session_data.get('stations'):
            ...
            if not df_s.empty:
                ...
                params = params.rename(columns={'Estado': 'Condición'})
                escribir_y_formatear(params.round(2), 'Parametros_Estaciones')

                df_s_export = clean_df_excel(df_s).rename(columns={'Estado': 'Condición'})
                escribir_y_formatear(df_s_export, 'Estaciones_Crudo')
...
def render_app_logic(data, mode="vivo"):
    ...
    if mode == "vivo" and start_dt:
        ...
        if not ya:
            st.markdown(
                f'<div class="alerta-colas">🚨 REGISTRA LA COLA: {curr_franja} 🚨</div>',
                unsafe_allow_html=True
            )
    ...
    with t1:
        ...
        if not readonly:
            ...
            if cp3.button("▶ Iniciar Pedido", type="primary", use_container_width=True):
                ...
                st.session_state.orders.append({
                    'ID': f"P-{st.session_state.order_counter:03d}",
                    'Canal': n_can,
                    'Items': n_itm,
                    'Estado': 'Ordering',
                    'Inicio_ts': time.time(),
                    'Inicio_dt': now,
                    'Hora Inicio': now.strftime("%H:%M:%S"),
                    'Fin Ordering': '-',
                    'Hora Entrega': '-',
                    'Duración Total(s)': 0
                })
        ...
        if data['orders']:
            ...
            if not comp.empty:
                st.dataframe(
                    clean_df_excel(comp[['ID', 'Canal', 'Items', 'Hora Inicio', 'Fin Ordering', 'Hora Entrega', 'Duración Total(s)']]).iloc[::-1],
    ...
    with t2:
        ...
        if not readonly:
            ...
            s_e = ic2.selectbox("Condición del componente:", ["Listo/disponible", "A pedido"])
            if ic3.button("▶ Iniciar Medición", type="primary", use_container_width=True):
                st.session_state.stations.append({
                    'ID': f"E-{len(st.session_state.stations)+1:03d}",
                    'Estación': s_n,
                    'Estado': s_e,
                    'Fase': 'Corriendo',
                    'Inicio_ts': time.time(),
                    'Hora Inicio': datetime.now(BOGOTA_TZ).strftime('%H:%M:%S')
                })
                st.rerun()
        ...
        for e in [e for e in st.session_state.stations if e['Fase'] == 'Corriendo']:
            with st.container(border=True):
                ...
                if st.button("🛑 Terminar", key=f"ef_{e['ID']}"):
                    ...
                    e['Duración(s)'] = round(time.time()-e['Inicio_ts'], 2)
                    e['Nota'] = nt
                    st.rerun()
        ...
        if data['stations']:
            ...
            if not comp_s.empty:
                st.dataframe(
                    comp_s[['ID', 'Estación', 'Estado', 'Duración(s)', 'Nota']]
                    .rename(columns={'Estado': 'Condición'})
                    .iloc[::-1],
                    use_container_width=True
                )
    ...
    with t3:
        ...
        if data['events']:
            st.dataframe(pd.DataFrame(data['events']).iloc[::-1], use_container_width=True)
    ...
    with t4:
        ...
        if data['orders'] and len(data['orders']) > 0:
            ...
            try:
                fig = px.line(
                    ...
                    y='Pedidos',
                    color='Canal',
                    color_discrete_map=MC_COLORS,
                    markers=True,
                    title="Curva de Demanda por Canal"
                )
        ...
        if data['stations']:
            ...
            if not comp_s.empty:
                ...
                st.info("Termina al menos una medición de estación para ver el resumen.")
        else:
...
if st.session_state.view_session:
    ...
    st.download_button(
        ...
        export_excel_pro(s['data'], s['info']),
        f"Reporte_{s['info']['fecha']}.xlsx",
        use_container_width=True
    )
            ...
            for s in reversed(st.session_state.history):
                with st.container(border=True):
                    ...
                    bc1.download_button(
                        "💾 Bajar Excel",
                        export_excel_pro(s['data'], s['info']),
                        f"Data_{s['info']['fecha']}.xlsx",
                        key=f"ex_{s['info']['start_dt']}"
                    )
    ...
    if h2.button("⏹ FINALIZAR SESIÓN", type="primary"):
        st.session_state.history.append({
            ...
            "data": {
                ...
                'queues': list(st.session_state.queues),
                'stations': list(st.session_state.stations),
                'capacity': list(st.session_state.capacity),
    ...
    render_app_logic(
        {
            ...
            'queues': st.session_state.queues,
